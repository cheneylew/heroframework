...
class Link_model extends CI_Model {
	...
	*/
	function gen_routes_file () {
		...
		$lang_str = $this->Language_model->get_language_string_list();
		...
		if (!empty($routes)) {
			...
			//Homepage rewrite rules
			$file .= "\$route['((en|tc|sc))/index'] = 'frontpage';\n";
			$file .= "\$route['((en|tc|sc))'] = 'frontpage';\n";

			//Content page rewrite rules
			foreach ($routes as $route => $path) {
				$file .= '$route[\'(('.$lang_str.'))/' . $route . '\'] = \'' . $path . '\';' . "\n";
