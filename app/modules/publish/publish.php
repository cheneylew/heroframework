...
class Publish extends Module {
	...
	*/
	...
	{
		$this->CI->admin_navigation->child_link('publish',10,'发布文章',site_url('admincp/publish/create'));
		...
		foreach ($result->result_array() as $type) {
			$this->CI->admin_navigation->child_link('publish',$weight,'&#149; 管理 ' . $type['content_type_friendly_name'],site_url('admincp/publish/manage/' . $type['content_type_id']));
			$weight++;
		}
		
		$this->CI->admin_navigation->child_link('publish',50,'文章话题',site_url('admincp/publish/topics'));
		$this->CI->admin_navigation->child_link('publish',60,'内容模型',site_url('admincp/publish/types'));
	}
