...
class Content_model extends CI_Model
{
	...
	*/
	function new_content ($type, $user, $title = '', $url_path = '', $topics = array(), $privileges = array(), $publish_date = FALSE, $custom_fields = array()) {
		...
		if($this->input->post("is_new")==="0" && $this->input->post("link_id")!=false && $this->input->post("link_id")!=""){
			$link_id = $this->input->post("link_id");
		}else if (!empty($title)) {// OC change end
	...
	*/
	function update_content ($content_id, $title = '', $url_path = '', $topics = array(), $privileges = array(), $publish_date = FALSE, $custom_fields = array()) {
		...
		if (!empty($title)) {
			if (empty($url_path)) {
				$this->load->helper('clean_string');
				$url_path = clean_string(is_array($title)?current($title):$title);
			}
			...
			$this->link_model->update_title($content['link_id'], is_array($title)?current($title):$title);
		...
		$update_fields = array(
							'content_title' => $title,
							'language_id' => $this->input->post("language_id"),//Also update language_id
							'content_privileges' => (is_array($privileges) and !in_array(0, $privileges)) ? serialize($privileges) : '',
							'content_modified' => date('Y-m-d H:i:s'),
							'content_topics' => (is_array($topics) and !empty($topics)) ? serialize($topics) : ''
						);
	...
	*/
	function get_content_id($url_path) {
		...
		if($this->uri->segment(1) != false){//Get Current Page Language
		...
		$this->db->where_in('language_id', array($language_id,0));//Handle other content not support multi language
		// OC change end
	...
	// Reason: add multiple language
	/**
	* Get Translate Content with same link_id
	*
	* Returns content ID from a link_id
	*
	* @param string $link_id
	* 
	* @return object The content, or FALSE
	*/
	function get_brother_content($link_id) {
		$this->db->select('content.language_id,content.link_id, content.content_id,content.content_title, links.link_url_path');
		$this->db->join('content','links.link_id = content.link_id','left');
		$this->db->from('links');
	...
	*/
	function get_contents ($filters = array(), $counting = FALSE) {
		// cache check!
		if (false && isset($this->CI->cache) and (!isset($filters['keyword']) and !isset($filters['sort_dir']) or (strtolower($filters['sort_dir']) != 'rand()' and strtolower($filters['sort_dir']) != 'random'))) {
			$caching = TRUE;
		...
		// OC change start 2012-08-24 by Oscar
		// Reason: add multiple language
		if (isset($filters['language_id'])) {//Search Content by language_id
			$this->db->where('content.language_id',$filters['language_id']);
		}
		// OC change end
		...
		foreach ($result->result_array() as $content) {
			...
			$this_content = array(
								...
								// OC change start 2012-08-24 by Oscar
								// Reason: add multiple language
								'language_id' => $content['language_id'],//Assign language_id
								'language_name' => $this->Language_model->get_language_name($content['language_id']),//Assign language_name
								'url' => site_url($this->Language_model->get_language_code($content['language_id'])."/".$content['link_url_path']),//Modify the url to exactly language_path for view button use
								// OC change end
			...
			if (isset($custom_fields) and !empty($custom_fields)) {
				foreach ($custom_fields as &$field) {
					...
					// OC change start 2012-08-24 by Oscar
					// Reason: New Custom Field
					$field["values"] = $content[$field['name']];//Get the custom field value
					// OC change end
				}
				reset($custom_fields);
			}
		...
		if (!empty($custom_fields)) {
			$contents[0]['custom_fields'] = $custom_fields;
		}
