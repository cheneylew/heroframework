...
<div class="sidebar">
	...
	<div class="sidebar_content">
		<a href="<?=site_url('admincp/users/edit/' . $user['id']);?>">编辑简介</a><br />
		<a href="<?=site_url('admincp/billing/new_subscription/' . $user['id']);?>">添加描述</a><br />
		<? if ($user['suspended'] == FALSE) { ?>
			<a href="<?=site_url('admincp/users/suspend_user/' . $user['id']);?>">暂停用户</a><br />
		<? } else { ?>
			<a href="<?=site_url('admincp/users/unsuspend_user/' . $user['id']);?>">启用用户</a><br />
		<? } ?>
		<a href="<?=site_url('admincp/users/send_user_email/' . $user['id']);?>">发邮件</a><br />
		<a href="<?=site_url('admincp/users/login_to_account/' . $user['id']);?>">登录账户</a>
	</div>
	
	<h3>记录</h3>
	
	<div class="sidebar_content">
		<a href="<?=dataset_link('admincp/users/logins/',array('username' => $user['username']));?>">登录历史</a><br />
...
<div style="float: left; width: 70%;">
	<h1><?=$user['username'];?></h1>
	<h2 class="cat user">用户信息</h2>
	<ul class="data">
		<li><span class="tag">用户名</span> <?=$user['username'];?></li>
		<li><span class="tag">完成用户名</span> <?=$user['last_name'];?>, <?=$user['first_name'];?></li>
		<li><span class="tag">邮箱</span> <?=$user['email'];?></li>
		<li><span class="tag">用户组</span><?=$user['show_usergroups'];?></li>
		...
		<? } } ?>
		<li><span class="tag">&nbsp;</span> <a href="<?=site_url('admincp/users/edit/' . $user['id']);?>">编辑简介</a></li>
	</ul>
