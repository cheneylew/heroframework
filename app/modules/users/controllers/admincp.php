...
class Admincp extends Admincp_Controller {
	...
	function index () {
		$this->admin_navigation->module_link('添加用户/管理员',site_url('admincp/users/add'));
		$this->admin_navigation->module_link('配置用户列表',site_url('admincp/users/configure_list'));
		...
		// add actions
		$this->dataset->action('发邮件','admincp/users/send_email');
		$this->dataset->action('暂停','admincp/users/suspend');
		$this->dataset->action('退出','admincp/users/unsuspend');
		$this->dataset->action('删除','admincp/users/delete');
	...
	function profile ($id) {	
		...
		// navigation
		$this->admin_navigation->module_link('编辑简介',site_url('admincp/users/edit/' . $user['id']));
	...
	function add () {
		...
		$form->text('用户名', 'username', '', FALSE, TRUE, FALSE, TRUE);
		$form->text('邮箱', 'email', '', FALSE, TRUE, 'email@example.com', TRUE);
		$form->password('密码', 'password', '', TRUE, TRUE);
		$form->password('确认密码', 'password2', '密码长度必须至少为6个字符', TRUE, TRUE);
		$form->fieldset('Usergroup');
		$form->dropdown('用户组','usergroups',$usergroup_options, array($this->usergroup_model->get_default()), TRUE, TRUE);
		$form->checkbox('管理员','is_admin','1',FALSE);
		$form->fieldset('Profile Information');
		$form->names('名字', '', '', FALSE, TRUE);
	...
	function edit ($id) {
		...
		$form = new Admin_form;
		$form->fieldset('系统信息');
		$form->text('用户名', 'username', $user['username'], FALSE, TRUE, FALSE, TRUE);
		$form->text('邮箱', 'email', $user['email'], FALSE, TRUE, 'email@example.com', TRUE);
		$form->password('密码', 'password', '', FALSE, TRUE);
		$form->password('确认密码', 'password2', '留空以保留当前密码。密码长度必须至少为6个字符', FALSE, TRUE);
		$form->fieldset('用户组');
		$form->dropdown('用户组','usergroups',$usergroup_options, $user['usergroups'], TRUE, TRUE);
		$form->checkbox('管理员','is_admin','1',$user['is_admin']);
		$form->fieldset('简介');
		$form->names('名字', $user['first_name'], $user['last_name'], FALSE, TRUE);
	...
	function groups () {
		$this->admin_navigation->module_link('添加用户组',site_url('admincp/users/group_add'));
	...
	function group_edit ($id) {
		...
		$form = new Admin_form;
		$form->fieldset('Group Information');
		$form->text('名称', 'name', $group['name'], FALSE, TRUE, FALSE, TRUE);
