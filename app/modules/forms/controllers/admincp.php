...
class Admincp extends Admincp_Controller {
	...
	function add () {
		...
		//When it is a new translation, copy the content from original content 
		$translation_content = null;
		if($this->input->get("translation")){
			$this->load->model('form_model');
			$translation_content = $this->form_model->get_form($this->input->get("translation"));
		}
	...
	function edit ($id) {
		...
		$topics->options($this->Language_model->get_language_option())
			   ->name('language_id')
			   ->label('Language')
			   ->value($form["language_id"]);//Form selected language_id
		
		$standard = $this->form_builder->output_admin();
		$brother_list = $this->form_model->get_brother_form($form["link_id"]);
