...
class Form_model extends CI_Model
{
	...
	* @param string $template Output template file (default: form.thtml)
	...
	function new_form ($title, $url_path, $text, $button_text, $redirect, $email = '', $privileges = array(), $template = 'form.thtml') {
		...
		if($this->input->post("link_id")!=false && $this->input->post("link_id")!=""){//If it is a translate content so don't add into link table
			$link_id = $this->input->post("link_id");
			$brother_form = $this->get_brother_form($link_id);
			$table_name = $brother_form[0]["form_table_name"];
		}else{// OC change end
			...
			$this->dbforge->add_field('`language_id` INT(255) NOT NULL');// OC change start 2012-08-24 by Oscar, Reason: add multiple language
		...
		$insert_fields = array(
							'link_id' => $link_id,
							'language_id' => $this->input->post("language_id"),//Assign language_id, OC change start 2012-08-24 by Oscar, Reason: add multiple language
	...
	* @param string $template Output template file (default: form.thtml)
	...
	function update_form ($form_id, $title, $url_path, $text, $button_text, $redirect, $email = '', $privileges = array(), $template = 'form.thtml') {
		...
		$update_fields = array(
							'form_title' => $title,
							'language_id' => $this->input->post('language_id'),// OC change start 2012-08-24 by Oscar, Reason: add multiple language
							'form_text' => $text,
							'form_email' => $email,
							'form_button_text' => $button_text,
							'form_redirect' => $redirect,
	...
	*/
	function delete_form ($form_id) {
		...
		$list = $this->get_brother_form($form['link_id']);//Get the translate content, OC change start 2012-08-24 by Oscar, Reason: add multiple language
		
		// delete custom fields
		$this->load->model('custom_fields_model');
		$this->custom_fields_model->delete_group($form['custom_field_group_id']);
		
		if(!$list){//If no other translate content, delete the record table and link, OC change start 2012-08-24 by Oscar, Reason: add multiple language
			// delete link
			$this->load->model('link_model');
	...
	*/
	function get_form_id($url_path) {
		$language_id = $this->Language_model->get_default_language_code();//Get Default language_id
		if($this->uri->segment(1) != false){
			$criteria["language_code"] = $this->uri->segment(1);
			if($this->Language_model->is_language($criteria["language_code"])){
				$language_id = $this->Language_model->get_current_language_id();
			}
		}
	...
	// Reason: add multiple language
	/**
	* Get Tradition Content
	*
	* Returns content ID from a link_id
	*
	* @param string $link_id
	* 
	* @return int The content, or FALSE
	*/
	function get_brother_form($link_id) {
		$this->db->select('forms.language_id,forms.link_id, forms.form_id,forms.form_title, links.link_url_path,forms.form_table_name');
		$this->db->join('forms','forms.link_id=links.link_id','left');
		$this->db->from('links');
		$this->db->where('links.link_id',$link_id);
		
		$content_list = $this->db->get()->result_array();
		
		$list = $this->Language_model->get_language_list();
		
		foreach($list as &$val){
			if($content_list){
				foreach($content_list as $val2){
					if($val["language_id"] == $val2["language_id"]){
						$val = array_merge($val2, $val);
					}
				}
			}
		}
	...
	function get_forms ($filters = array()) {
		...
		foreach ($result->result_array() as $row) {
			...
			$forms[] = array(
						...
						'language_name' => $this->Language_model->get_language_name($row['language_id']),
						'url' => site_url($this->Language_model->get_language_code($row['language_id'])."/".$row['link_url_path']),
						// OC change end
						'link_id' => $row['link_id'],
