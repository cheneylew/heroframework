...
class File_list_fieldtype extends Fieldtype {
	/**
	* Constructor
	*
	...
	*/
	function __construct () {
		...
		$this->enabled = TRUE;
		$this->fieldtype_name = 'File List';
		$this->fieldtype_description = 'File List';
	...
	*/
	function output_admin () {
		...
		// build HTML
		$return = "<li>";
		$return .= "<script src='branding/default/js/jquery-ui-1.8.2.min.js'></script><script src='branding/custom/js/fieldtypes.upload.js'></script> <style>@import 'branding/custom/css/fieldtypes.upload.css';@import 'branding/default/css/black-tie/jquery-ui-1.8.2.custom.css';</style><label>".$this->name."</label>";


		$return .= "<input type='button' class='button' onclick='openKCFinderLink(\"".$this->name."\", \"".$this->data["multi_files"]."\")' value='Upload & select'/>";
		
		$return .= "<table id='".$this->name."' class='file_list' border='1'>";
		$return .= "<thead><th></th><th>Title</th><th>File Name</th><th>Delete</th></thead>";
		$return .= "<tbody>";

		if(is_array($this->value) && isset($this->value['path'])){
			foreach($this->value['path'] as $key=>$val){
				$return .= "<tr>";
				$return .= "<td><span class='ui-icon ui-icon-arrowthick-2-n-s'></span></td>";
				$return .= "<td><input name='".$this->name."[title][]' type='text' value='".$this->value['title'][$key]."'/></td>";
				$return .= "<td><input name='".$this->name."[path][]' type='hidden' value='".$this->value["path"][$key]."'/>
				<a href='".$this->value['path'][$key]."' target='_blank'>".basename($this->value['path'][$key])."</a></td>";
				$return .= "<td><a onclick='deleteUploadFile(this)' class='button'>Delete</a></td>";
				$return .= "</tr>";
			}
		}
		$return .= "</tbody>";
		...
		$return .= '<div class="help">'.$this->help.'</div>';
		$return .= "</li>";
	...
	*/
	function output_frontend () {
		...
		foreach($this->value["path"] as $key=>$val){
			$content[$key]["title"] = $this->value["title"][$key];
			$content[$key]["path"] = $this->value["path"][$key];
		}
	...
	*/
	function field_form ($edit_id = FALSE) {
		...
		$multi_files = $this->CI->form_builder->add_field('checkbox');
	    $multi_files->label('Allow Multiple Files')
	    	 ->name('multi_files')
	    	 ->help('Allow user select more than 1 files');
	    ...
	    if (!empty($edit_id)) {
			...
			if(isset($field['data']['multi_files'])){
				$multi_files->value($field['data']['multi_files']);
			}
	    }	  
	...
	*/
	function field_form_process () {
		...
		return array(
					...
					'data' => array(
								'multi_files' => ($this->CI->input->post('multi_files')) ? TRUE : FALSE
							),
