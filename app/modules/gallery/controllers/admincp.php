...
class Admincp extends Admincp_Controller {
	...
	function index () {		
		...
		$this->admin_navigation->module_link('发布新的 Gallery',site_url('admincp/gallery/create/' . $type_id));
	...
	function create () {
		...
		$title->text('标题','title','',FALSE,TRUE,FALSE,TRUE);
		...
		$privileges->name('privileges')
				   ...
				   ->default_value(0)
				   ->label('允许的成员资格组')
				   ->help('如果选择了一个或多个组，则此内容将要求此组中的用户查看它。这使你能够对将用户移动到此群组的订阅和产品收取费用。');
