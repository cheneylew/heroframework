...
function smarty_function_menu ($params, $smarty) {
	...
	if (empty($links)) {
		return 'There are no links in this menu (' . $menu['name'] . ').  Go to your 菜单管理 in the control panel and add more links.';
	}
		...
		if ($display_this_item == TRUE) {
			if ($link['special_type'] == FALSE) {
				...
				if ($link['external_url']) {
					...
					$url = site_url($smarty->CI->uri->segment(1)."/".$link['link_url_path']);
				}
				...
				if ($link['special_type'] == 'home') {
					$menu_items[$link['id']] = array(
												'text' => $link['text'],
												'url' => site_url().$smarty->CI->uri->segment(1),
