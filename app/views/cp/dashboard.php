...
<? if (module_installed('billing','store','coupons')) { ?>
	<div id="dash_stats" rel="day">
		<div id="date_selector">
			<ul>
				<li><input type="radio" name="date_select" value="day" /> <span>今天</span></li>
				<li><input type="radio" name="date_select" value="week" /> <span>本周</span></li>
				<li><input type="radio" name="date_select" value="month" /> <span>本月</span></li>
			</ul>
		</div>
		<div class="stat">
			<div class="wrap">
	...
	<div class="quick_publish">
		<ul>
			<? if (empty($quick_publish)) { ?>
				<li><a href="<?=site_url('admincp/publish/types');?>">管理 Content Types</a></li>
			<? } else { ?>
				<? foreach ($quick_publish as $type) { ?>
				<li><a href="<?=$type['link'];?>">&#43; 发布 <?=$type['name'];?></a></li>
				<? } ?>
			<? } ?>
		</ul>
		<a class="more" href="<?=site_url('admincp/publish/create');?>">发布其它类型文章 &gt;</a>
	</div>
